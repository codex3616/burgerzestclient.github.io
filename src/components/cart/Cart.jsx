...
//another component inside card component...
const CartItem = ({ title, value, img, increment, decrement }) => {
  return (
    <div className="cartItem">
      <div>
        <h4>{title}</h4>
        <img src={img} alt="burgerImg.." />
      </div>
      <div>
...
const Cart = () => {
  ...
  const {
    cartItems: {
      cheeseBurger: { quantity: cheeseBurger }, //this means know we can call cheeseBurger = cheeseBurger.quantity
  ...
  const decrement = (item) => {
    // if (cheeseBurger === 0 && vegCheeseBurger === 0 && burgerWithFries === 0) {
  ...
  }, [orderItems, shippingCharges, tax, total, subTotal]);
  return (
    <>
      <section className="cart">
        <main>
          <CartItem
            title={"Cheese Burger"}
            img={burger1}
            value={cheeseBurger} //redux store value
